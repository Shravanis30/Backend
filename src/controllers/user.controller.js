...
const updateUserAvatar = asyncHandler(async(req, res) => {
    ...
    .json(
        new ApiResponse(
            200, 
            user,
            "Avatar file uploaded successfully"
        )
    )
})
...
const updateUserCoverImage = asyncHandler(async(req, res) => {
    ...
    .json(
        new ApiResponse(
            200, 
            user,
            "Cover image file uploaded successfully"
        )
    )
})
    ...
    const channel = await User.aggregate([
        ...
        },
        {
            $lookup: {
                from: "subscriptions",
                localField: "_id",
                foreignField: "channel", // to get whom i have subscribed 
                as: "subscribers"
            }
        },
        {
            $lookup:{
                from: "subscriptions",
                localField: "_id",
                foreignField: "subscriber", // to get what i have subscriberd 
                as: "subscribedTo"
            }
        },
        {
...
const getWatchHistory = asyncHandler(async(req, res) => {
    const user = await User.aggregate([
        {
            $match:{
                // interview question what we get from req.user.id (we didnt get the id of the user instead we get string of the object the correct way to get id as as followed)
